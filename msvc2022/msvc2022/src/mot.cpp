...
void Mot::main()
{
    ...
    while (true) // continue until finish
    {
        ...
        if (!q_yolo2buffer.empty()) {//get data from YOLO. -> postprocess first.
            ...
            //get data
            yolo2buffer = q_yolo2buffer.front();
            q_yolo2buffer.pop();
            count_yolo++;
            if (count_yolo >= 5) {
                rois = yolo2buffer.rois;
                labels = yolo2buffer.labels;
                frame = yolo2buffer.frame;
                frameIndex = yolo2buffer.frameIndex;
